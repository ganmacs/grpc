...
class ServerImpl < RouteGuide::Service
  ...
  def get_feature(point, _call)
    name = @feature_db[{ 'longitude' => point.longitude, 'latitude' => point.latitude }] || ''
  ...
  def record_route(call)
    ...
    call.each_remote_read do |point|
      count += 1
      name = @feature_db[{'longitude' => point.longitude,
...
def main
  ...
  port = '0.0.0.0:50051'
  s = GRPC::RpcServer.new(interceptors: [MyInterceptor.new])
...
class MyInterceptor < GRPC::Interceptor
  def request_response(request: nil, call: nil, method: nil)
    GRPC.logger.info('start reqeust_response')
    yield.tap do |resp|
      result = { code: resp.code, request: request.to_h, response: resp.metadata[:response] }
      GRPC.logger.info("finish reqeust_response #{result}")
    end
  end

  def client_streamer(requests: nil, call: nil, method: nil, metadata: nil)
    GRPC.logger.info('start client_streamer')
    call.each_remote_read.each do |req|
      GRPC.logger.info(response: req.to_h)
    end

    yield.tap do |resp|
      result = { code: resp.code, response: resp.metadata[:response] }
      GRPC.logger.info("finish client_streamer #{result}")
    end
  end

  def server_streamer(request: nil, call: nil, method: nil, metadata: nil)
    GRPC.logger.info('start server_streamer')
    yield(stream_handler: StreamHandler.new(call)).tap do |resp|
      result = { code: resp.code, request: request.to_h }
      GRPC.logger.info("finish server_streamer #{result}")
    end
  end

  def bidi_streamer(requests: nil, call: nil, method: nil, metadata: nil)
    GRPC.logger.info('start bidi_streamer')
    call.each_remote_read.each do |req|

    end

    yield(stream_handler: StreamHandler.new(call)).tap do |_resp|
      result = { requests: requests.to_h }
      # can't get code...
      GRPC.logger.info("finish bidi_streamer #{result}")
    end
  end
  ...
  class StreamHandler
    ...
    def remote_send(req, marshalled = false)
      @handler.remote_send(req, marshalled).tap do
        GRPC.logger.info(response: req.to_h)
      end
    end
