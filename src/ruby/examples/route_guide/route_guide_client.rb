...
def main
  stub = RouteGuide::Stub.new('localhost:50051', :this_channel_is_insecure, interceptors: [MyInterceptor.new])
...
class MyInterceptor < GRPC::Interceptor
  def request_response(request: nil, call: nil, method: nil, metadata: nil)
    puts nil, '<=====requesst======================================================= OUTPUT START HERE'
    yield # => nil
  end

  def server_streamer(request: nil, call: nil, method: nil, metadata: nil)
    puts nil, '<=====server steamer======================================================= OUTPUT START HERE'
    yield
  end

  def bidi_streamer(requests: nil, call: nil, method: nil, metadata: nil)
    puts nil, '<=====bidi======================================================= OUTPUT START HERE'
    hander = Wrapper2.new(call)
    yield(stream_handler: hander)
  end

  def client_streamer(requests: nil, call: nil, method: nil, metadata: nil)
    puts nil, '<=====client steamer======================================================= OUTPUT START HERE'
    hander = Wrapper2.new(call)
    yield(stream_handler: hander)
  end
  ...
  class Wrapper2
    ...
    def remote_send(req, marshalled = false, metadata: {})
      @handler.remote_send(req, marshalled, metadata: metadata).tap do
        GRPC.logger.info("this is our loggin #{req.to_h}")
      end
    end
